        ...
        <div class="card">
          <div class="cardTitle">
            ...
            <div class="row">
              ...
              <select id="weeklyDate" style="width:auto; min-width:220px;"></select>
        ...
        <div class="card">
          ...
          <div id="historyFilters" class="card" style="background: rgba(0,0,0,.14); box-shadow:none; margin-bottom:12px;">
            <div class="form" style="gap:10px;">
              ...
              <div class="field half" id="wrapHistoryWeek" style="display:none;">
                <label for="historyWeek">주 선택</label>
                <input id="historyWeek" type="week" />
              </div>
    ...
    (() => {
      ...
      function weekDatesFromYMD(ymd) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          out.push(ymdFromDate(x));
        }

        if (CLAMP_WEEK_TO_MONTH) {
          const baseYM = (ymd || "").slice(0, 7);
          return out.filter((d) => d.slice(0, 7) === baseYM);
        }
        return out;
      }
      ...
      const historyWeekEl = $("#historyWeek");
      ...
      function getFilteredEntriesHistory() {
        ...
        return entries.filter((e) => {
          if (period === "day") {
            ...
            if (CLAMP_WEEK_TO_MONTH) {
              const baseYM = weekRange.start.slice(0, 7);
              if (monthISO(e.date) !== baseYM) return false;
            }
      ...
      // 필터 입력 반응
      [historyDayEl, historyWeekEl, historyMonthEl, historyQueryEl].forEach((el) => {
        el.addEventListener("input", () => renderHistory());
      });

      // ✅ year 직접 입력: 숫자만/4자리만 + 4자리면 적용
      ...
      historyYearEl.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
          e.preventDefault();
          if ((historyYearEl.value || "").trim().length === 4) renderHistory();
        }
      });
      ...
      const yearPrev = $("#yearPrev");
      const yearNext = $("#yearNext");
      const yearClose = $("#yearClose");
      const yearRangeLabel = $("#yearRangeLabel");

      const YEAR_MIN = 1900;
      const YEAR_MAX = 2100;
      const YEAR_PAGE = 20;
      let yearPageStart = Math.max(YEAR_MIN, (new Date().getFullYear() - 5));

      function clampYearPageStart(v){
        const maxStart = YEAR_MAX - (YEAR_PAGE - 1);
        return Math.min(Math.max(v, YEAR_MIN), maxStart);
      }

      function renderYearGrid(){
        const start = clampYearPageStart(yearPageStart);
        const end = Math.min(YEAR_MAX, start + YEAR_PAGE - 1);
        yearRangeLabel.textContent = `${start} ~ ${end}`;

        const selected = Number((historyYearEl.value || "").trim());
        yearGrid.innerHTML = "";
        ...
        for (let y = start; y <= end; y++) {
          ...
          if (Number.isFinite(selected) && selected === y) btn.classList.add("primary");

          btn.addEventListener("click", () => {
            historyYearEl.value = String(y);
      ...
      function openYearModal() {
        const current = Number((historyYearEl.value || "").trim());
        if (Number.isFinite(current) && current >= YEAR_MIN && current <= YEAR_MAX) {
          yearPageStart = clampYearPageStart(current - Math.floor(YEAR_PAGE/2));
        } else {
          yearPageStart = clampYearPageStart(new Date().getFullYear() - 5);
        }
        renderYearGrid();
      ...
      historyYearEl.addEventListener("click", openYearModal);
      yearPrev.addEventListener("click", () => { yearPageStart = clampYearPageStart(yearPageStart - YEAR_PAGE); renderYearGrid(); });
      yearNext.addEventListener("click", () => { yearPageStart = clampYearPageStart(yearPageStart + YEAR_PAGE); renderYearGrid(); });
      ...
      function renderDashboard() {
        ...
        for (const e of monthEntries) {
          if (e.type === "expense") { expense += e.amount || 0; expenseList.push(e); }
          else { income += e.amount || 0; incomeList.push(e); }
        }
      ...
      function initDailyDateOptions() {
        ...
        dailyDatePrettyEl.textContent = prettyYmdKo(base);
      ...
      function renderDailySummary() {
        ...
        dailyDatePrettyEl.textContent = prettyYmdKo(selected);

        const dayEntries = entries.filter((e) => e.date === selected);
        let income = 0, expense = 0;
        for (const e of dayEntries) { if (e.type === "expense") expense += e.amount || 0; else income += e.amount || 0; }
        const balance = income - expense;
      ...
      function syncWeeklyDatesFromDaily() {
        const base = dailyDateEl.value || todayISO();
        const days = weekDatesFromYMD(base);

        weeklyDateEl.innerHTML = "";
        for (const ymd of days) {
          const opt = document.createElement("option");
          opt.value = ymd;
          opt.textContent = prettyYmdKo(ymd);
          weeklyDateEl.appendChild(opt);
        }

        weeklyDateEl.value = days.includes(base) ? base : (days[0] || "");
        weeklyDatePrettyEl.textContent = prettyYmdKo(weeklyDateEl.value);
        renderWeeklySummary();
      }

      function renderWeeklySummary() {
        const selected = weeklyDateEl.value || dailyDateEl.value || "";
        const days = weekDatesFromYMD(selected);
      ...
      dailyDateEl.addEventListener("change", () => { renderDailySummary(); syncWeeklyDatesFromDaily(); });
      weeklyDateEl.addEventListener("change", () => { weeklyDatePrettyEl.textContent = prettyYmdKo(weeklyDateEl.value); renderWeeklySummary(); });
      dashMonthEl.addEventListener("input", () => { historyMonthEl.value = dashMonthEl.value; renderAll(); });
      ...
      $("#exportCsv").addEventListener("click", async () => {
        ...
        const rows = entries.map((e) => [
          e.date || "", e.type || "", e.category || "", String(e.amount || 0),
          (e.memo || "").replace(/\r?\n/g, " "),
        ]);
        const csvBody = [header, ...rows].map((cols) => cols.map(csvCell).join(",")).join("\n");
        const csvWithBom = "\ufeff" + csvBody; // 엑셀 한글 깨짐 방지
        const filename = `ledger-${monthISO(todayISO())}-${Date.now()}.csv`;
        const r = await smartExport(filename, csvWithBom, "text/csv;charset=utf-8");
        toast(r.mode === "share" ? "CSV 공유로 내보냄" : r.mode === "clipboard" ? "CSV가 복사됨" : "CSV 백업 텍스트 열림");
      });
      ...
      function init() {
        ...
        historyWeekEl.value = toISOWeekString(new Date());
        historyYearEl.value = String(new Date().getFullYear());

        setType("expense");
        refreshCategoryOptions();

        initDailyDateOptions();
